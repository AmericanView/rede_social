            	...
            	<div id="formulario">
                    ...
                    <?php
						if(isset($_SERVER['REQUEST_METHOD']) AND $_SERVER['REQUEST_METHOD'] == 'POST'){
							...
							if($nome == '' OR strlen($nome)<2){
							...
							}elseif($sobrenome=='' OR strlen($sobrenome)<3){
								...
								if($verificar->execute(array($email))){
									if($verificar->rowCount()>=1){
										...
										echo 'Sua senha deve ter mais de 4 caracteres';
									}elseif(strtolower($captcha) <> strtolower($_SESSION['captchaCadastro'])){
										echo 'O código digitado no captcha não corresponde a imagem';
									}else{
										$senhaInsert = sha1($senha);
										$nascimento = "$ano-$mes-$dia";
										
										$inserir = DB::getConn()->prepare("INSERT INTO `usuarios` SET `email`=?, `senha`=?, `nome`=?, `sobrenome`=?, `sexo`=?, `nascimento`=?, `likes`=?, `unlikes`=?, `cadastro`=NOW()");
										
										if($inserir->execute(array($email,$senhaInsert,$nome,$sobrenome,$sexo,$nascimento,$likes,$unlikes))){
											echo 'cadastro realizado com sucesso, <a href="index.php">CLIQUE AQUI</a>';
										}
									}
								}
                    ...
                    <form name="cadastro" action="" method="post">
                    	<div>
                            ...
                            <div class="inputFloat">
                                ...
                                <select style="display:none;" name="likes"><option>1</option></select>
                                <select style="display:none;" name="unlikes"><option>1</option></select>
                            </div><!--inputFloat-->
                      	</div>
