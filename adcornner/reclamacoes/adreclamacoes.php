...
<div id="lol">
<h1 align="center">Exibição de Reclamações</h1>
<?php
...
$resultado = mysql_query("SELECT * FROM `listreclamacoes`");
...
<form method="post" action="responderreclamacao.php" ><input type="hidden" name="id" value="<?php echo $escrever['id'] ?>" /><input type="hidden" name="dataresposta" value="<?php echo date('d/m/Y').' as '.date('H:i:s') ?>" /><textarea placeholder="Responda a reclamação..." name="responder" rows="10" cols="50"></textarea><br /><input type="submit" value="Responder" /></form><?php } ?></td></tr>
</table>
<?php } ?>
<?php } ?></div>
</body>
